...
NETWORK_ROLES = {"deployment", "tenant", "compartment", "network_zone", "trust_zone", "availability_zone", "cluster", "namespace", "host", "workload", "endpoint", "gateway", "load_balancer", "database", "queue", "cache", "worker", "identity", "secret_store"}
...
SHAPE_BY_ROLE = {"system": "box", "component": "box", "service": "box", "interface": "ellipse", "data_store": "cylinder", "external_actor": "circle", "external_system": "box", "library": "box", "boundary": "box"}
...
def _container_header(line: str) -> Container | None:
    ...
    if not match:
        return None
    return Container(match.group("id"), match.group("label") or match.group("plain_label") or match.group("id"))
...
def parse_logical_flowchart(source: str) -> DiagramIR:
    ...
    for line_number, line in enumerate(lines[1:], start=2):
        ...
        if container:
            ...
            container = Container(container.id, container.label, container.role, parent)
        ...
        if edge_parts:
            ...
            edge_label, edge_role = _split_edge_role(edge_label)
    ...
    ir = DiagramIR("logical", direction, tuple(nodes.values()), tuple(edges), tuple(containers))
